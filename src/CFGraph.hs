...
toCFG (FnDef _ _ args (Block stmts)) = do
   cfg <- execStateT (createBlock stmts (-1) [Map.empty]) (CFGraph { blocks = Map.fromList [(0, addFunParams args)], nextL = 1})
...
createBlock :: (MonadTrans m, MonadState CFGraph (m IO)) => [Stmt] -> Label -> [EEnv] -> m IO Label

createBlock (st@(BStmt (Block s)):ss) n ev = do
   next' <- createBlock ss n ev
   next <- createBlock s next' (Map.empty:ev)
   return next      
createBlock (st@(CondElse e sb1 sb2):ss) l ev = do
   lcond <- nextLabel
   --call recursively
   next <- createBlock ss l ev
   bt <- createBlock [sb1] next (Map.empty:ev)
   bf <- createBlock [sb2] next (Map.empty:ev)
   let (e1, _) = evalExpr e Map.empty
   let bcond = case e1 of
         ELitTrue -> CFBlock {stmts=[], jumpTo=JJump bt, inEdges=[], env=ev}
         ELitFalse -> CFBlock {stmts=[], jumpTo=JJump bf, inEdges=[], env=ev}
         x -> CFBlock {stmts=[], jumpTo=(JCond {true=bt, false=bf, cond=e}), inEdges=[], env=ev}
   g <- get
   let m = blocks g
   let ifmap = Map.fromList [(lcond, bcond)]
   put $ CFGraph{blocks = (Map.union m ifmap), nextL = (nextL g)}
   return lcond    
createBlock (st@(Cond e s):ss) n ev = do
   lcond <- nextLabel
   --call recursively
   next <- createBlock ss n ev
   bt <- createBlock [s] next (Map.empty:ev)
   let (e1, _) = evalExpr e Map.empty
   let bcond = case e1 of
         ELitTrue -> CFBlock {stmts=[], jumpTo=JJump bt, inEdges=[], env=ev}
         ELitFalse -> CFBlock {stmts=[], jumpTo=JJump next, inEdges=[], env=ev}
         x -> CFBlock {stmts=[], jumpTo=(JCond {true=bt, false=next, cond=e}), inEdges=[], env=ev}
   g <- get
   let m = blocks g
   let ifmap = Map.fromList [(lcond, bcond)]
   put $ CFGraph{blocks = (Map.union m ifmap), nextL = (nextL g)}
   return lcond    
createBlock (st@(While e s):ss) l ev = do
   lcond <- nextLabel
   next <- createBlock ss l ev
   body <- createBlock [s] lcond (Map.empty:ev)
   ...
   put $ CFGraph{blocks = (Map.union m ifmap), nextL = (nextL g)}
   return lcond    
createBlock (st@(Decl t items):ss) n ev = do
   l <- nextLabel
   --let (stm, rest, ev') = joinBlockStmts (s:ss) (head ev)--Map.empty
   let (ev', s') = addNewVarsE t items (head ev) []
   next <- createBlock (s'++ss) n (ev':tail ev)
   g <- get
   let m = blocks g
   let b = CFBlock {stmts=[], jumpTo=(JJump next), inEdges=[], env = (ev':tail ev)}  
   put $ CFGraph{blocks = (Map.insert l b m),  nextL = (nextL g)}
   return l    
createBlock (s:ss) n ev = do
   l <- nextLabel
   let (stm, rest, ev') = joinBlockStmts (s:ss) (head ev)--Map.empty
   next <- createBlock rest n (ev':tail ev)
   g <- get
   let m = blocks g
   let b = CFBlock {stmts=stm, jumpTo=(JJump next), inEdges=[], env = (ev':tail ev)}  
   put $ CFGraph{blocks = (Map.insert l b m),  nextL = (nextL g)}
   return l    
createBlock [] l _ = do
   return l
...
joinBlockStmts s ev = joinBS [] s ev
   where
      joinBS acc rest@(s@(Decl t items):ss) ev = (reverse acc, rest, ev)--joinBS (s' ++ acc) ss ev' where
