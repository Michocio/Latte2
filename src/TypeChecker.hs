...
hasReturnOrVoid :: [Stmt] -> Type -> (Bool, [Stmt])
...
checkBlock t (Block stmts) = do
   (fenv, venv) <- get
   put (fenv, Map.empty:venv)
   mapM (checkStmt t) stmts
   put (fenv, venv)
   return ()

--finds the most recent type of a variable
findType :: Ident -> VEnv -> Maybe Type
findType id [] = Nothing
findType id (x:xs) = case Map.lookup id x of
   Nothing -> findType id xs
   ty@(Just t) -> ty
...
checkDoesNotExist stmt t id@(Ident s) = do 
   (fenv, venv) <- get
   case Map.lookup id (head venv) of
      Nothing -> return ()
      Just t -> do
         lift $ hPutStr stderr "ERROR\n"
         error $ (printCodeLine stmt) ++ " Variable '" ++ s ++ "' already declared!\n" 
   put (fenv, (Map.insert id t (head venv):(tail venv)))    
