...
replacePhi = do
   ...
   where
      ...
      replaceP p@(CPhi reg phi@(Phi t args)) = do
         lift $ putStrLn $ "Zamieniam args w phi"
         args' <- replace' args []
         ...
         where 
            ...
            replace' (((PhiPlaceholder i), nr):xs) l = do
               ...
               let (reg, _) = case findValueInBlockEEnv (env bl) id of--Map.lookup id (env bl) of
                     Just x -> x
                     Nothing -> fromMaybe (error $ "No such var in block env: " ++ show nr ++ "id: " ++ show id) $ Map.lookup id v
               replace' xs $ l ++ [(reg, nr)]
            replace' (x:xs) l = replace' xs $ l ++ [x]
            replace' [] l = return l
      replaceP e = do
      --   lift $ putStrLn $ "replaceP " ++ show e
         return e

findValueInBlockEEnv :: [EEnv] -> Ident -> Maybe (CExpr, Type)
findValueInBlockEEnv [] _ = Nothing
findValueInBlockEEnv (x:xs) id = case Map.lookup id x of
   Nothing -> findValueInBlockEEnv xs id
   Just x -> Just x 
...
compileCFGBlock nr = do
   ...
   let s = stmts bl
   let ve = env bl
   let (venv', senv') = addVarTypes venv senv $ Map.assocs $ head ve
...
compileStmt nr s@(Ass id e) = do
   ...
   let bl' = CFBlock {
      ...
      jumpTo=jumpTo bl,
      inEdges=inEdges bl,
      env=(Map.insert id (r, t) $ head $ env bl):(tail $ env bl) --dodalam head i tail
   }
   let cfg' = CFGraph {
...
findValue id (-1) = do 
   ...
   let bl = m Map.! (0)
   let ve = env bl
   let val = findValueInBlockEEnv ve id
   lift $ putStrLn $ "findValue id: " ++ show id ++ " -1\n" ++ show ve ++ "\n" ++ show val 
   case val {-Map.lookup id ve-} of
      Just (Exp e, _) -> do
         x1 <- compileExpr 0 e
         return [x1]
      Just (e, _) -> return [e]
...
findValue id nr = do
   ...
   let val = findValueInBlockEEnv ve id
   lift $ putStrLn $ "findValue id: " ++ show id ++ "\n" ++ show ve ++ "\n" ++ show val 
   case val {-Map.lookup id ve-} of
   ...
   --case Map.lookup id ve of
      ...
      Nothing ->
         do
            ...
            let b = CFBlock{stmts=stmts bl, jumpTo=jumpTo bl, inEdges=inEdges bl, env=(Map.insert id ((PhiPlaceholder pnr), t) (head ve)):(tail ve)}
            ...
            if length exps' == 1 then do
               ...
               let b' = CFBlock{
                  ...
                  jumpTo=jumpTo bl', 
                  inEdges=inEdges bl', 
                  env=(Map.insert id (h, t) (head (env bl'))):(tail $ env bl')
               }
               let c' = CFGraph{
            ...
            --jak dostaje >1 rzecz to robie phi
               ...
               let b' = CFBlock{
                  ...
                  jumpTo=jumpTo bl', 
                  inEdges=inEdges bl', 
                  env=(Map.insert id ((CReg r), t) (head $ env bl')):(tail $ env bl')
               }
               let c' = CFGraph{
                  blocks=Map.insert nr b' m',
                  nextL=nextL cfg
...
getType nr (EVar id) = do
   ...
   --case Map.lookup id ve of
      Nothing -> do
         ...
         case Map.lookup id venv of --moze tu tego teraz byc nie powinno? nie wiem, sie okaze
            Just (e, t) -> do
               return (e, t) 
            Nothing -> do
               let ins = inEdges bl
      ...
      Just (e, Void) -> do
         ...
         let b = CFBlock {
            ...
            jumpTo=jumpTo bl',
            inEdges=inEdges bl',
            env=(Map.insert id (e, t) (head $ env bl')):(tail $ env bl')
         }
         let cfg'' = CFGraph {
            blocks=Map.insert nr b m',
            nextL=nextL cfg'
